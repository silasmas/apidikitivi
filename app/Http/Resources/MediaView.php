    ...
    public function toArray(Request $request): array
    {
        ...
        if (!is_null($media)) {
            return [
                ...
                'cover_url' => $media->cover_url != null ? (!empty($_SERVER['HTTPS']) ? 'https' : 'http') . '://admindikitivi.jptshienda.com/public' . $this->cover_url : null,
                'thumbnail_url' => $media->thumbnail_url != null ? (!empty($_SERVER['HTTPS']) ? 'https' : 'http') . '://admindikitivi.jptshienda.com/public' . $this->thumbnail_url : null,
                'price' => $media->price,
                'for_youth' => $media->for_youth,
                'is_live' => $media->is_live,
                'belongs_to' => $media->belongs_to,
                'type' => Type::make($media->type),
                'categories' => Category::collection($media->categories)->unique('id')->all(),
                'created_at_ago' => timeAgo($media->created_at->format('Y-m-d H:i:s')),
                'created_at' => $media->created_at->format('Y-m-d H:i:s'),
                'updated_at_ago' => timeAgo($media->updated_at->format('Y-m-d H:i:s')),
